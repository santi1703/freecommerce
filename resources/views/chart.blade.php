                ...
                @if($products->isNotEmpty())
                    <table class="table-auto w-full">
                        ...
                        <tr>
                            ...
                            <th class="border border-black px-4 py-2 text-black">{{ ucfirst(__('app.product.total')) }}</th>
                            <th class="border border-black px-4 py-2 text-black">{{ ucfirst(__('app.product.remove')) }}</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($products as $product)
                            <tr>
                                ...
                                <td class="border border-black px-4 py-2 text-black font-medium">{{ $product->quantity }}</td>
                                <td class="border border-black px-4 py-2 text-black font-medium">
                                    ${{ $product->quantity * $product->price }}
                                </td>
                                <td class="border border-black px-4 py-2 text-black font-medium">
                                    <form method="POST" action="{{ route('chart.remove', ['id' => $product->id]) }}">
                                        @csrf
                                        <button
                                            class="bg-red-300 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
                                            {{ ucfirst(__('app.product.remove')) }}
                                        </button>
                                    </form>
                                </td>
                            </tr>
                        ...
                        <tr>
                            <td colspan="3"
                                class="border border-black px-4 py-2 text-black font-medium text-center">
                                {{ __('app.grand_total', ['amount' => $total]) }}
                            </td>
                            <td colspan="4"
                                class="border border-black px-4 py-2 text-black font-medium text-center">
                                <form method="POST" action="{{ route('purchase.create') }}">
                                    ...
                                    <button
                                        ...
                                        {{ ucfirst(__('app.purchase.confirm')) }}
                                    </button>
                                </form>
                            </td>
                        </tr>
